...
## Решение:
1) На Gitlab создал репозиторий с именем test1 (через интерфейсе gitlab) Внимание(!). Изначально данны репозиторий назывался test1, но для удобства после проходения курся я его переименовал в Gi_helper
